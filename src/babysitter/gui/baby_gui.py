...
class BabyMonitorGui(QWidget):
    def __init__(self, cfg: GuiConfig, parent=None):
        ...
        self.video_view = VideoViewWrapper()
        self.describe_box_label = LabelWrapper("Baby Nanny").set_style(color="blue", font_size=18)
        ...
        # Layouts
        controls_layout = QHBoxLayoutWrapper([self.start_button, self.stop_button])  # Removed describe_now_button
        right_layout = QVBoxLayoutWrapper([self.describe_box_label, self.describe_box,
                                           self.prompt_box_label, self.prompt_box,
                                           controls_layout])
        root_layout = QHBoxLayoutWrapper([self.video_view, right_layout])

        self.setLayout(root_layout.get_widget())


        # Controller
        self.controller = BabyMonitorController()
        #self.controller = None  # Placeholder until we implement the controller
